...
export async function POST(request: NextRequest) {
  console.log('=== VIDEO-DATA API ROUTE CALLED ===')
  ...
  try {
    ...
    return NextResponse.json({ 
      totalViews, 
      videoCount,
      message: 'Real data fetched from TikTok API via unified endpoint'
    })
    
  } catch (error) {
    console.error('Error in video-data API route:', error)
    return NextResponse.json({ 
...
export async function GET() {
  console.log('=== VIDEO-DATA GET ROUTE CALLED ===')
  return NextResponse.json({ 
    message: 'Video-data API endpoint is working',
    status: 'success',
